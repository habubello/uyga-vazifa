...
def register_page(request):
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            message = render_to_string('emaill/verify_email_message.html', {
                'request': request,
                'user': user,
...
class RegisterPage(FormView):
    template_name = 'ecomerce/register.html'
...
def verify_email_confirm(request, uidb64, token):
    ...
    return render(request, 'emaill/verify_email_confirm.html')


def verify_email_complete(request):
    return render(request, 'emaill/verify_email_complete.html')